...
func _ready():
	GameState_.skeleton_alive = 1
	enemy = get_tree().get_root().get_node("Main/Gladiator")
...
func take_damage(damage: float):
	if seconds < seconds_to_live: 
		...
		print("Skeleton hp: " + str(current_health))
		if current_health <= 0:
			die()
...
func _physics_process(delta):
	...
	if seconds > seconds_to_live:
		...
		if enemy and global_position.distance_to(enemy.global_position) < weapon_range:
			#print("in range")
			if time_since_last_attack >= attack_speed and GameState_.gladiator_alive == 1:
				#print("asd")
